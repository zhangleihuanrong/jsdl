...
exports.loadImage = function (url) {
    ...
    .then((originImage) => {
        const ow = originImage.bitmap.width;
        const oh = originImage.bitmap.height;
        ...
        let nh = 256;
        if (oh > ow) {
            nh = Math.ceil(oh / (1.0 *ow) * nh);
        }
        else if (ow > oh)  {
            nw = Math.ceil(ow / (1.0 * oh) * nw);
        }
        console.log(`    ---- resize it to size: ${nw} X ${nh}`);
        const scaleImage = originImage.resize(nw, nh);

        const w = 224;
        const h = 224;
        const starty = Math.ceil((nh - h) / 2.0);
        const startx = Math.ceil((nw - w) / 2.0);
        console.log(`    ---- crop it to size: ${w} X ${h}`);
        const image = scaleImage.crop(startx, starty, w, h);

        image.quality(100).write('./temp.jpg');

        const mean = {r: 128.0, g: 128.0, b: 128.0};
        const std = {r: 128.0, g: 128.0, b: 128.0};
        const matrix = new Float32Array(3* w * h);
        const RED = 2 * w * h;      // Make target GBR
        const GREEN = w * h;
        const BLUE = 0 * w * h;
        image.scan(0, 0, w, h, (x, y, idx) => {
            ...
            //assert(rgba.a == 255);
            matrix[ti + RED] = (rgba.r - mean.r) / std.r; //image.bitmap.data[ idx + 0 ]; //red
            matrix[ti + GREEN] = (rgba.g - mean.g) / std.g; //image.bitmap.data[ idx + 1 ]; //green
            matrix[ti + BLUE] = (rgba.b - mean.b) / std.b; //image.bitmap.data[ idx + 2 ]; //blue
        });
        return matrix;
    });
};

exports.sampleImageUrls = [
    ...
    { text: 'sampleflower', value: 'https://cdn.pixabay.com/photo/2015/02/10/21/28/flower-631765_1280.jpg'},
...
exports.imagenetClasses = imagenetClasses;

exports.imagenetClassesTopK = function(classProbabilities, k) {
    k = k || 5;
